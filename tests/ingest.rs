...
fn prose_bundle() -> Vec<u8> {
    ...
    bundle::tar_append(
        &mut tar,
        "manifest.json",
        bundle::render_manifest(&manifest).as_bytes(),
    );
...
fn good_bundles_file_per_sender_and_reruns_skip() {
    ...
    assert!(
        inbox.join("aabbccdd00112233").join(&name).exists(),
        "inbox is never mutated"
    );
    let _ = std::fs::remove_dir_all(&base);
}
...
fn bad_bundles_quarantine_with_reasons() {
    ...
    let reason =
        std::fs::read_to_string(quar.join("aabbccdd00112233/prose.tar.zst.reason.txt")).unwrap();
    assert!(
        reason.contains("journal.txt is not free-text-clean"),
        "{reason}"
    );
    assert!(
        quar.join("aabbccdd00112233/garbage.tar.zst.reason.txt")
            .exists()
    );
...
fn name_collision_with_different_content_quarantines() {
    ...
    assert_eq!(
        std::fs::read(lib.join("s1").join(&name)).unwrap(),
        bytes,
        "library untouched"
    );
    let _ = std::fs::remove_dir_all(&base);
